...
export const parseDateValue = (fieldValue: string): DateParseResult => {
	...
	const naturalDateMatch = fieldValue.match(
		/^(?:Date)?([a-z]+)\s*(?:-\s*)?(\d{1,2})(?:st|nd|rd|th)?\s*(?:,\s*)?(\d{4})$/i,
	);
	...
	if (naturalDateMatch) {
		try {
			...
			if (monthIndex !== -1) {
				...
				const paddedDay = day?.padStart(2, '0') || 'Err';
				const now = new Date();
			...
			try {
				...
				switch (pattern.format) {
					...
					case DateFormat.DD_MM_YY: {
						...
						const hours = now.getHours().toString().padStart(2, '0');
						const minutes = now.getMinutes().toString().padStart(2, '0');
						processedValue = `${day?.padStart(2, '0') || 'Err'}${month?.padStart(2, '0') || 'Err'}${fullYear}-${hours}${minutes}`;
