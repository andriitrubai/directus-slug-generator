 ...
 * @param options.separator The character to use as a separator (default: '-')
 * @param options.lowercase Whether to convert the slug to lowercase (default: true)
...
export const createSlug = (
	...
	if (isUrl) {
		...
		if (protocol && rest) {
			...
			const cleanedRest = rest
				...
				.replaceAll(/[^a-z0-9\-/.:]/gi, '') // Allow dots, slashes, and colons for URLs
				.trim();
		...
		slug = slug
			.normalize('NFD') // Normalize to decomposed form
			.replaceAll(/[\u0300-\u036F]/g, '') // Remove any remaining diacritics
			.replaceAll(/[^a-z0-9\-/ ]/gi, '') // Remove special chars, but allow slashes and hyphens
			.trim()
	...
	if (!isUrl) {
		...
		slug = slug.replaceAll(/[-/]+/g, separator);

		// Then ensure we don't have multiple consecutive separators
		slug = slug.replaceAll(new RegExp(`${separator}+`, 'g'), separator);
	}
	...
	if (!isUrl) {
		slug = slug.replaceAll(new RegExp(`^${separator}|${separator}$`, 'g'), '');
	}
